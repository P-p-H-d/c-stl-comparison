...
const config_func_t table[] = {
  ...
  { 340, "USet Longest(dict)", C_N_FIND_SEQ, 0, bench_find_longest2, 0},
  { 341, "USet Longest(dict oa)", C_N_FIND_SEQ, 0, bench_find_longest, 0},
  ...
  { 601,    "Queue(MPMC)", C_N_THR_QUEUE, 0, test_queue, 0},
  { 602,    "Queue(Buffer P2)", SIZE_LIMIT+C_N_THR_QUEUE, 0, test_buffer, 0},
  { 603,"Queue(MPMC P2)", SIZE_LIMIT+C_N_THR_QUEUE, 0, test_queue, 0},
  { 604,"Queue(SPSC P2)", C_N_THR_QUEUE, 0, test_queue_single, 0},
  { 605,"Queue(Shared ptr)",C_N_THR_QUEUE, 0, test_queue_concurrent, 0},
  { 610,"Queue(SPSC Bulk)",C_N_THR_QUEUE, 0, test_queue_single_bulk, 0},
  { 700,         "Hash (M_HASH)", C_N_SEQ_BYTES, test_hash_prepare, test_hash, test_hash_final},
  { 701,      "Hash (Core Hash)", C_N_SEQ_BYTES, test_hash_prepare, test_core_hash, test_hash_final},
  { 800, "serial-bin STR", C_N_SERIAL_JSON, bench_vector_string_init, bench_vector_string_bin_run, bench_vector_string_clear},
  { 810, "serial-bin STR.big", C_N_SERIAL_JSON, bench_vector_string_init_big, bench_vector_string_bin_run, bench_vector_string_clear},
