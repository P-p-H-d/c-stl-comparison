...
test_dict2(size_t  n)
{
  ...
  unsigned int s = 0;
  for (size_t i = 0; i < n; i++) {
    flat_hash_map<unsigned long, unsigned long>::iterator it = dict.find(rand_get());
    if (it != dict.end())
      s += it->second;
  }
  g_result = s;
...
test_dict2_linear(size_t  n)
{
  ...
  unsigned int s = 0;
  for (size_t i = 0; i < n; i++) {
    flat_hash_map<unsigned long, unsigned long>::iterator it = dict.find(i);
    if (it != dict.end())
      s += it->second;
  }
  g_result = s;
...
test_dict_big(size_t  n)
{
  ...
  for (size_t i = 0; i < n; i++) {
    char_array_s s1;
    sprintf(s1.a, "%u", rand_get());
    flat_hash_map<char_array_s, char_array_s>::iterator it = dict.find(s1);
    if (it != dict.end())
      s ++;
  }
  g_result = s;
...
test_dict_str(size_t  n)
{
  ...
  for (size_t i = 0; i < n; i++) {
    string s1 = static_cast<ostringstream*>( &(ostringstream() << rand_get()) )->str();
    flat_hash_map<string, string>::iterator it = dict.find(s1);
    if (it != dict.end())
      s ++;
  }
  g_result = s;
