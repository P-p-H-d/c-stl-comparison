...
The results are archived in git so that you can look at the history of the different runs. Thanks to that, the best rank and worst rank of the 10 previous runs are extracted: this enable detecting external interference (for example, some bench are dependent also on the performance of the kernel to allocate pages).
