...
// WORKAROUND: Needs to provide function and not encapsulation macro
...
int main(void)
{
  mpz_z_struct z;                                     //WORKAROUND: we shall not clear the z object, as its ownership has been lost.
  vec_mpz_z_struct a = vec_mpz_z_struct_init();
  mpz_init_set_ui(z.Z, 17);
  vec_mpz_z_struct_push_back(&a, z);                  //WORKAROUND: We need to provide a fresh new initialized object for each push
  mpz_init_set_ui(z.Z, 42);
  vec_mpz_z_struct_push_back(&a, z);
  mpz_init_set_ui(z.Z, 9);
  vec_mpz_z_struct_push_back(&a, z);
  a.compare = mpz_z_struct_compare;                   //WORKAROUND: Needs to set the compare function directly in the struct?
